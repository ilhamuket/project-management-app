    ...
    <form action="{{ route('projects.update', $project->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <select id="jenis_konstruksi" name="jenis_konstruksi" 
                ...
                <option value="Pengerasan" {{ old('jenis_konstruksi', $project->jenis_konstruksi) == 'Pengerasan' ? 'selected' : '' }}>Pengerasan</option>
                <option value="Lainnya" {{ old('jenis_konstruksi', $project->jenis_konstruksi) == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
            </select>
            ...
            @if ($project->dokumen_kontrak)
                <div class="mb-2">
                    @foreach (json_decode($project->dokumen_kontrak, true) as $dokumen)
                        <a href="{{ asset('storage/' . $dokumen) }}" class="text-blue-600 hover:underline" target="_blank">{{ basename($dokumen) }}</a><br>
                    @endforeach
                </div>
            @endif
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let map;
    let marker;
    let isLocationSelected = {{ !empty($project->koordinat) ? 'true' : 'false' }};

    // Default center (Jakarta) or existing coordinates if available
    @php
        $coordinates = explode(',', $project->koordinat);
        $latitude = isset($coordinates[0]) ? trim($coordinates[0]) : '2.0872';
        $longitude = isset($coordinates[1]) ? trim($coordinates[1]) : '117.3736';
    @endphp
